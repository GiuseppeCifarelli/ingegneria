...
public class ServerApplicativo {
	...
	/*
	 * Il server per prima deve poter permettere il login all'amministratore e finch� non va a buon fine il login, le operazioni non sono disponibili
	 */
	public void EseguiServerApplicativo() throws Exception {
		...
		// creazione stream di input da clientSocket
				...
				System.out.println("Login effettuato");
					...
					while(!fineSessione) {
						...
						System.out.println("avanza Richiesta 1");
						out.println(avanzaRichiesta);
						...
						/*
						System.out.println("avanza Richiesta 2");
						out.println(avanzaRichiesta);
						*/
					}
	...
	public void ModificaEvento() {
		...
		try {
			...
			eventiStore.Update(connessioneDB, in.readLine(),Integer.parseInt(in.readLine()),in.readLine(), in.readLine(), in.readLine(), data, Double.parseDouble(in.readLine()), Integer.parseInt(in.readLine()),in.readLine());
		} catch (IOException e) {
	...
	public void CancellazioneEvento(){
		try {
			out.println("aspetta");
			boolean done = eventiStore.Delete(connessioneDB, Integer.parseInt(in.readLine()));
			System.out.println("valore di done : " + done);
			out.println("avanti");			
			if(!done) out.println("Errore");
			else out.println("corretto");
			} catch ( Exception e) {
			System.out.println("Errore cancellazione:"); e.printStackTrace();
			out.println("Errore");//informo il client dell'errore verificatosi
		}
	}
